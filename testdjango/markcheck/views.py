...
from .models import ThresholdData1, ThresholdData_O, Sat_Curves, ThresholdData_OP
from .forms import ThresholdForm1, SatForm, ThresholdFormO
...
def olevel(request):
    ...
    if request.method=="POST":
        form= ThresholdFormO(request.POST)
        print(form)
        if form.is_valid():
            subject= form.cleaned_data['subject']
            variant= str(form.cleaned_data['variant'])
    ...
    return render(request, 'olevel.html', {'form': ThresholdFormO})
